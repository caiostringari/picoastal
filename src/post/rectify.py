...
def save_as_geotiff(grid_x: np.ndarray, grid_y: np.ndarray, res: float,
                    rgb: np.ndarray, epsg: int, outfile: str):
    """
    ...
    nx = rgb.shape[0]
    ny = rgb.shape[1]
    geotransform = [grid_x.min(), res, 0, grid_y.min(), 0, res]
    ...
    srs.ImportFromEPSG(int(epsg))  # EPSG:28356 - GDA94 / MGA zone 56 - Projected
...
def main():
    ...
    parser.add_argument("--resolution",
                        ...
                        dest="resolution",
                        default=1,
                        help="Grid resolution in meters. Default is 1m.")
    ...
    grid_x, grid_y = np.meshgrid(np.arange(bbox[0], bbox[0] + bbox[2], res),
                                 np.arange(bbox[1], bbox[1] + bbox[3], res))
    ...
    # output
    save_as_geotiff(grid_x, grid_y, res, rgb, args.epsg, args.output)
